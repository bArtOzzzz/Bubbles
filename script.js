class CirclesIntersection {
  ...
  updateCircles() {
    ...
    this.circles.forEach(circle => {
      ...
      if (circle.yPos + circle.radius > this.h && circle.velocityX > 0 || circle.yPos < circle.radius && circle.velocityY < 0) {
  ...
  drawIntersection(sideA, sideB, sideC, dx, dy, circle) {
    ...
    let pointOneX = circle.xPos + Math.cos(angleCorrection - angleOfRotation) * sideA;
    let pointOneY = circle.yPos + Math.sin(angleCorrection - angleOfRotation) * sideA;
    let pointTwoX = circle.xPos + Math.cos(angleCorrection + angleOfRotation) * sideA;
    ...
    this.drawCircle(pointTwoX, pointTwoY, this.intersectionDotRadius, this.intersectionFillcolor, this.intersectionStrokecolor);
  }
  clearCanvas() {
